...
8. Check materialized overlay drift with `packport configport check <state-root> <generated> <output> --profile <profile> --target <target> --pack <pack>`.
9. Materialize selected instructions with `packport configport instructions apply <state-root> <pack-root> <output> --profile <profile> --target <target> --pack <pack> --scope <scope>`.
10. Report which files changed, whether drift checks passed, and where configport state was written.
